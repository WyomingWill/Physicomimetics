...
class ParticleFormationActivity : AppCompatActivity() {
    ...
    private val infoText = """
        ...
        Multiple particles self-organize into a triangular or square lattice using a generalized Lennard-Jones force law - the same style of force that governs molecular bonds, repulsive at short range and attractive at longer range, with a well-defined "desired separation" distance between neighbors.

        HOW IT WORKS

        Each particle senses nearby neighbors within its field of view and computes a force based on their distance. The force smoothly transitions from strongly repulsive (too close) to attractive (too far), settling near the desired separation. An optional goal force pulls the whole formation toward a fixed point, and green obstacles exert a simple linear repulsive force.

        HOW TO USE IT

        Tap Setup to create particles, then Start to watch them self-organize. Touch the graphics area to place obstacles. Toggle Formation switches between triangular and square lattices; Toggle Goal turns the attractive pull on and off. Add, Remove, and Disable change the particle count live. The Lennard-Jones Constant and Power sliders shape the force curve; Desired Separation sets the target spacing.
    """.trimIndent()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_particle_formation)
    ...
    private fun setupControls() {
        ...
        findViewById<Slider>(R.id.sliderLJConstant).addOnChangeListener { _, value, _ ->
            simulation.params.lennardJonesConstant = value
            findViewById<android.widget.TextView>(R.id.labelLJConstant).text =
                "Lennard-Jones Constant: " + value.toInt()
        }
