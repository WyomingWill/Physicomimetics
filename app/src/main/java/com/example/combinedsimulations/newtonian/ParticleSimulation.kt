...
class ParticleSimulation {
        ...
        for (p in particles) {
            ...
            for (other in particles) {
                ...
                if (modifiedR < view * params.desiredSeparation) {
                    var f = params.gravitationalConstant * p.mass * other.mass / modifiedR.pow(params.power)
                    if (f > params.forceMaximum) f = params.forceMaximum
                    if (modifiedR > params.desiredSeparation) {
                        p.force.x += f * (dx / r)
                        p.force.y += f * (dy / r)
                    } else {
                        p.force.x -= f * (dx / r)
    ...
    private fun updateCenterOfMass() {
        val entities = if (isGoalEnabled) particles + goals + obstacles else particles + obstacles
    ...
    fun getPhaseTransitionG(): Float {
        val dp = params.desiredSeparation.pow(params.power)
        return if (isSquareFormation) {
            params.forceMaximum * dp / ((2f * sqrt(2f)) + 2f)
        } else {
            params.forceMaximum * dp / (2f * sqrt(3f))
        }
    }

    fun getAbandonGoalForce(): Float {
        val dp = params.desiredSeparation.pow(params.power)
        var gf = (params.gravitationalConstant * heavyMass) / dp
        if (gf > params.forceMaximum) gf = params.forceMaximum
        return gf
    }
